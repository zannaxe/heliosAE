...
export const PowerShellTool = buildTool({
  ...
  async call(input: PowerShellToolInput, toolUseContext: Parameters<Tool['call']>[1], _canUseTool?: CanUseToolFn, _parentMessage?: AssistantMessage, onProgress?: ToolCallProgress<PowerShellProgress>): Promise<{
    ...
    try {
      ...
      // HeliosAE hints protocol: CLIs/SDKs gated on CLAUDECODE=1 emit a
...
async function* runPowerShellCommand({
  ...
  if (!powershellPath) {
    // Pre-flight failure: pwsh not installed.
    // HeliosAE: provide OS-specific install instructions
    const isLinux = process.platform === 'linux'
    const installHint = isLinux
      ? '\n\nInstall PowerShell on Linux:\n  Ubuntu/Debian: sudo apt install -y powershell\n  Fedora/RHEL:   sudo dnf install -y powershell\n  Arch:          yay -S powershell-bin\n  Or via snap:   sudo snap install powershell --classic\n\nAlternatively, use BashTool for this task instead.'
      : '\n\nPowerShell could not be found. Make sure PowerShell is installed and in your PATH.'
    return {
      stdout: '',
      stderr: 'PowerShell is not available on this system.' + installHint,
      code: 0,
      interrupted: false
    };
  }
