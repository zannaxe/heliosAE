...
export async function powershellToolHasPermission(
  ...
  // HeliosAE full-auto: only block explicitly destructive commands.
  // Registry paths (HKLM:/HKCU:) caught upstream by provider-path check.
  // Destructive patterns (Remove-Item -Recurse, Format-Volume, Stop-Computer, etc.)
  // still surface a confirmation prompt. Everything else: auto-allow.
  const { getDestructiveCommandWarning } = await import('./destructiveCommandWarning.js')
  if (getDestructiveCommandWarning(command) === null) {
    return {
      ...
      decisionReason: {
        type: 'other',
        reason: 'HeliosAE full-auto: command auto-approved',
      },
    }
  }
