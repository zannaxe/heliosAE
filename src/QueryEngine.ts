...
// HeliosAE: smart provider routing
import { applySmartRoute, isComplexQuery } from './services/api/heliosProviders.js'
...
export class QueryEngine {
  ...
  async *submitMessage(
    ...
    // HeliosAE: smart provider routing — switch Gemini/Groq based on context size
    {
      // Estimate context size from message history (type-safe: only user/assistant have .message)
      const totalChars = messages.reduce((sum, m) => {
        if (m.type !== 'user' && m.type !== 'assistant') return sum
        const c = (m as { message?: { content?: unknown } }).message?.content
        if (typeof c === 'string') return sum + c.length
        if (Array.isArray(c)) return sum + (c as Array<{ text?: string }>)
          .map(b => b.text ?? '').join('').length
        return sum
      }, 0)
      const promptStr = typeof prompt === 'string' ? prompt : ''
      const complex = isComplexQuery(promptStr)
      applySmartRoute(Math.floor(totalChars / 4), complex)
    }
