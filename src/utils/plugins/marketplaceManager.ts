/**
 * Marketplace manager for HeliosAE plugins
 *
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a HeliosAE bug, shouldn't hit the error file.
    return {}
  }
}
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `HeliosAE version. Run 'claude marketplace remove ${name}' and ` +
